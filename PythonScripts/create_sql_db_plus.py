...
inpho = ["Please, find the location of the config.txt file. Click ok to continue to file path dialog box.", "Please, find the location of your geojson file. Click ok to continue to file path dialog box."]
...
database = database+ "/TSPdataBasedel.db"
jsonPath = inputs_paths[1]
        ...
        for key, value in sorted(values['properties'].iteritems()):
            ...
            if year in key:
                ...
                if  len(templist) == 6:
                    ...
                    try:
                        templist.append(values['properties']['uniqID'])
                    except KeyError:
                        templist.append(values['id'])
                    try:
    ...
    while startYear <= 2018:  #  loops to the number of years 33
        ...
        for i in range(end): # loops to the number of polygons 170 
            try:
                ...
                keys = ['b1', 'b2', 'b3', 'b4', 'b5', 'b7', 'cloud_cover', 'image_julday', 'image_year', 'plotid', 'project_id', 'selected', 'sensor', 'spectral_scaler', 'target_day','tcb', 'tcg','tcw', 'tsa', 'url']
                values = [templistB[i][0], templistB[i][1], templistB[i][2], templistB[i][3], templistB[i][4], templistB[i][5], 0, '217', int(templistB[i][6]), templistB[i][7].encode("utf-8"), 1100, 'null', 'LT5', 10000, 'undefined', 0, 0, 0, 999999, 'another.png']
...
def main():
    ...
    obser_val_dict = observation_value_dict(startYear, end, data) # chage here vvv
